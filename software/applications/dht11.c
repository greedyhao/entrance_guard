...
#define DHT11_DATA_PIN    GET_PIN(E, 2)

static uint8_t _temp = 0;
static uint8_t _humi = 0;

void dht11_get_temp_humi(uint8_t *temp, uint8_t *humi)
{
    *temp = _temp;
    *humi = _humi;
}
...
static void read_temp_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            {
                rt_enter_critical();
                _temp = (sensor_data.data.temp & 0xffff) >> 0;      // get temp
                _humi = (sensor_data.data.temp & 0xffff0000) >> 16; // get humi
                rt_exit_critical();
